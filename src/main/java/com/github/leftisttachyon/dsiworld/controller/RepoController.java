...
/**
 * A controller class that deals with operations with repositories.
 *
 * @author Jed Wang
 * @since 1.0.0
 */
...
public class RepoController {
    ...
    public String showRepoBlob(@SessionAttribute("user") User user, @PathVariable String id, @PathVariable String name,
        ...
        if (f.isDirectory()) {
            // dirs, files
            List<File> dirs = new ArrayList<File>(), files = new ArrayList<File>();
...
//                log.debug(chunk);
    ...
    public String showLog(@SessionAttribute User user, @PathVariable String id, @PathVariable String name,
        ...
        List<String[]> commitData = new LinkedList<String[]>();
